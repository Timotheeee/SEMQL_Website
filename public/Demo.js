...
//                found = true;
//            } else {
//                if (i === 0 && !contains(highlights, words))
//                    console.log("pushing words: " + words);
//                if (words.trim().split(" ").length > 1 && !contains(highlights, words))
...
//        if ($.inArray(el, highlights2) === -1)
//            highlights2.push(el);
//    });
//
//    if (i === 0)
//        console.log(highlights2);
//
...
function load(id) {
    ...
    if (text.includes("<select name"))
        removeSelects();
    text = $("#humanText" + id).html();
    for (var i = 0; i < 1000; i++) {
        text = text.replace(/<[^>]+>/, "");
    }
    $("#humanText" + id).html(text);
